...
| BUG-11 | `product.stock` read after `save()` of an `F()` expression — holds a `CombinedExpression`, not a number | orm.md — F() expressions |
...
| BUG-14 | `SerializerMethodField` crossing a relation — one query per serialized row under `many=True` | Class 1 — serializer `many=True` |
...
| MIG-09 | `db_index=True` on a previously unindexed column blocks writes for the index build | Class 2 — `AddIndexConcurrently`, `atomic = False` |
...
- **False positives count against a run.** `restock()` is deliberately correct:
  `filter().update(F(...))` is atomic, and re-reading with `Product.objects.get()`
  is the right way to get the value back. Flagging it is a defect in the review, not
  a bonus finding. `restock_and_report()` directly below it *is* broken — a run that
  cannot tell the two apart does not understand `F()`.
- **Findings not on this list are not automatically wrong.** The fixture contains
  further real defects that are not tagged (missing `@login_required` on `search` and
  `checkout`, no floor check on `stock`, `OrderTests` not inheriting from `TestCase`
  so the suite never runs it). A run that surfaces those is doing better than the key,
  not worse. Only flagging *correct* code counts against it.
